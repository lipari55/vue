<template>
    ...
    <!-- 修改用户对话框 -->
    <el-dialog title="修改用户" :visible.sync="ediDialogVisible"
      width="50%" @close="ediDialogClosed">
      <el-form :model="ediForm" :rules="editFormRules" ref="ediFormRef" label-width="70px">
...
export default {
  ...
  methods: {
    ...
    async showEdiDialog(id) {
      ...
      if (res.meta.status !== 200) {
        return this.$message.error('查用户信息失败！')
      }
    ...
    editUserInfo() {
      this.$refs.ediFormRef.validate(async valid => {
        ...
        const { data: res } = await this.$http.put(
          'users/' + this.ediForm.id,
          console.log(this.ediForm),
          {
            email: this.ediForm.email,
            mobile: this.ediForm.mobile
          }
        )
        if (res.meta.status !== 200) {
